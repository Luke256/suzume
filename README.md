...
- `cli:"0"` : 位置引数の順序を非負整数で指定します。値の小さい引数から処理され、非整数値はオプションを表します。
...
位置引数は指定した非負整数の小さい順に処理されます。オプション名と短縮名は重複できず、両者の間でも同じ名前は使えません。組み込みヘルプが使用する `help` と `h` も予約されています。不正なタグは `UseCommand` の呼び出し時にエラーになります。
